...
# (1=LC, 2=NT, 3=VU, 4=EN, 5=CR)
library(MASS)

# Phylogenetically structured predictor (Body Mass)
VCV <- ape::vcv.phylo(tree)
lambda_X <- 0.6
Sigma_X <- lambda_X * VCV + (1 - lambda_X) * diag(N)
X <- as.vector(mvrnorm(1, mu = rep(0, N), Sigma = Sigma_X))

# Phylogenetically structured error term
lambda_IUCN <- 0.75
tau_u <- 1 / 0.5
tau_e <- 1 / 0.3
...
# Linear predictor: larger body mass -> lower threat
eta <- -0.8 * log(abs(X) + 1) + phylo_error
cutpoints <- c(-1, 0, 1, 2) # Thresholds between categories

# Calculate probabilities using cumulative logit (proportional odds)
...
data_ordinal <- list(X = log(abs(X) + 1), IUCN = IUCN, K_IUCN = 5)

fit_ordinal <- phybase_run(
    data = data_ordinal,
    tree = tree,
    equations = list(IUCN ~ X),
...
# Phylogenetically structured predictor (Area)
VCV <- ape::vcv.phylo(tree)
lambda_area <- 0.6 # Phylogenetic signal in Area
Sigma_area <- lambda_area * VCV + (1 - lambda_area) * diag(N)
log_area <- as.vector(mvrnorm(1, mu = rep(0, N), Sigma = Sigma_area))
...
log_richness <- 2 + 0.5 * log_area + phylo_error
richness <- rpois(N, lambda = exp(log_richness))

data_poisson <- list(Area = log_area, Richness = richness)

fit_poisson <- phybase_run(
    data = data_poisson,
    tree = tree,
    equations = list(Richness ~ Area),
...
# (e.g., parasite loads, highly variable population counts)
library(MASS)

# Phylogenetically structured predictor (HostMass)
VCV <- ape::vcv.phylo(tree)
lambda_mass <- 0.5
Sigma_mass <- lambda_mass * VCV + (1 - lambda_mass) * diag(N)
log_host_mass <- as.vector(mvrnorm(1, mu = rep(0, N), Sigma = Sigma_mass))

# Phylogenetically structured error termfor parasites
...
log_mu <- 2 + 0.5 * log_host_mass + phylo_error
mu <- exp(log_mu)
parasite_count <- rnbinom(N, size = r_true, mu = mu)

data_nb <- list(HostMass = log_host_mass, Parasites = parasite_count)

fit_nb <- phybase_run(
    data = data_nb,
    tree = tree,
    equations = list(Parasites ~ HostMass),
