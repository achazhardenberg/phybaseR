...
**Format**: `beta_Response_Predictor` or `betaPredictor` (for simple models)

- `betaX` - Effect of X on Y in model `Y ~ X`
- `beta_Y_X` - Effect of X on Y (when X is also a response in another equation)
...
fit_custom <- phybase_run(
  ...
  tree = tree,
  equations = equations,
  monitor = c("betaX", "lambdaY", "alphaY")  # Specific parameters
)
```
