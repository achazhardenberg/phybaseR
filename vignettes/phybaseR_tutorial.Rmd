...
Because shines at testing causal paths, such as mediation: `X -> M -> Y`.
...
Because can account for measurement error in your variables.
...
fit_se <- because(
    ...
    tree = tree,
    equations = list(Y ~ X),
    variability = c(X = "se") # Tell Because that X has SEs
)

summary(fit_se)
```

**Note on DIC with measurement error**: When using measurement error (either SE or repeated measures), the DIC penalty will be inflated because Because creates latent "true" values for each species. The penalty will be approximately: structural parameters + N (number of species). This is expected behavior.

- For **model comparison**, use WAIC instead: `because(..., WAIC = TRUE)`
...
If you have raw data (multiple observations per species), Because can estimate the observation error directly.
...
# Because automatically handles the NAs and counts valid replicates per species.
```
...
# because_format_data automatically creates dummy variables
data_list <- because_format_data(data, tree = tree)
# Messages:
...
Because fully supports missing data (`NA`) in both response and predictor variables. It uses a **Latent Variable (GLMM)** approach to impute missing values while preserving phylogenetic signal.
...
# Because automatically detects NAs and handles them.
```
...
If you have a list of models to compare, you can run them all in parallel using `because_compare()`.
...
Set `WAIC = TRUE` in `because()` to calculate WAIC with standard errors automatically:
...
Compare fitted models using `because_compare()`, which returns a ranked table with WAIC, standard errors, differences, and Akaike weights.

#### Option 1: Compare Fitted Models

```{r waic_comparison, eval=FALSE}
# Fit competing models
fit_complex <- because(..., WAIC = TRUE)
fit_simple <- because(..., WAIC = TRUE)

# Compare results
comp <- because_compare(fit_complex, fit_simple)
print(comp)
...
Because can handle **unobserved (latent) variables** using two different approaches:
...
fit1 <- because(..., WAIC = TRUE)
fit2 <- because(..., WAIC = TRUE)
...
If you want to apply the same random effect structure to **all** equations in your model (e.g., a block effect or random intercept for Site across all species), you can use the `random` argument in `because`. This is more convenient than adding `(1|Group)` to every equation and ensures consistent handling in d-separation tests.
