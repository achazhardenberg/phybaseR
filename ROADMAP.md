...
-   **Proposed Syntax** (Matching TidySEM):
    ```r
    model <- tidy_because(data) %>%
      add_paths(Risk ~ Population + Climate) %>%
      set_distribution(Risk = "binomial") %>%  # 'because' specific extension
      estimate_jags()
    ```
-   **Key Features**:
    -   **`add_paths()`**: Incrementally build equations.
    -   **`set_distribution()`**: Explicitly handle families (Bernoulli, Poisson, etc).
